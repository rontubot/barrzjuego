...
export const Game: React.FC<GameProps> = ({ onBackToMenu }) => {
  ...
  const [wordsRotated, setWordsRotated] = useState(false); // Para rotación de 180° de palabras
  ...
  // Al sacar una nueva carta de desafío que tiene límite de tiempo
  useEffect(() => {
  ...
  // Funciones para sacar cartas aleatorias
  const drawBeat = () => {
    setBeatFlipped(false);
    // Simular retraso de animación
    setTimeout(() => {
  ...
  const openSpotify = (beat: BeatCard) => {
    // Intentamos abrir la URI nativa primero (funcionará en móviles con Spotify instalado)
    window.location.href = beat.spotifyUri;
    
    // Y abrimos el enlace web en una pestaña nueva como respaldo seguro
    setTimeout(() => {
  ...
  // Cálculo del retardo de animación para el metrónomo visual (60 / BPM)
  const bpmPulseDuration = activeBeat ? 60 / activeBeat.bpm : 0.67; // default ~90BPM
  ...
  return (
    <div className="game-container">
      ...
      <div className="game-header">
        ...
        <button className="btn-reset-game" onClick={() => { if(window.confirm('¿Reiniciar partida?')) { setTurn(1); setActiveBeat(null); setActiveChallenge(null); setActivePlayer('Freestyler A'); }}}>
          <RefreshCw size={16} />
        </button>
      </div>
        ...
        <div className="deck-column">
          ...
          {!activeBeat ? (
            ...
            <div className="deck-pile beat-pile glass-panel glow-teal" onClick={drawBeat}>
              ...
              {/* Efecto de cartas apiladas debajo */}
            ...
            <div className="card-container-3d">
              <div className={`card-inner-3d ${challengeFlipped ? 'flipped' : ''}`}>
                ...
                <div className="card-face card-front glow-teal" style={{ borderColor: 'var(--neon-teal)' }}>
                  ...
                  {/* Render de cuerpo según categoría */}
                  <div className="challenge-card-body">
                    ...
                    {/* Caso 3: Temáticas, Cypher, Terminaciones, 1v1, Sacrificio */}
